 ...
 * Initialized in {@link WBShop#init()}
 */
public class EconomyManager { // TODO: Ensure all wallets are always initialized
    ...
    /**
     * Get the instance of this class that is initialized by {@link WBShop}.
     *
     * @return The instance
     */
    public static EconomyManager getInstance() {
        return WBShop.ECONOMY_MANAGER;
    }
    ...
    private static EconomyManager loadFromFile() throws FileNotFoundException {
        ...
        return WBShop.GSON.fromJson(jsonString, EconomyManager.class);
    }
    ...
    public static EconomyManager makeNewManager() {
        ...
        try {
            ...
            WBShop.LOGGER.info("Could not find file to load player wallets from.");
        }
        if (manager == null) {
            WBShop.LOGGER.info("Initializing a new player wallet save file.");
    ...
    public int addBalance(UUID uuid, int amount) {
        if (!isValidEntry(uuid)) {
            WBShop.LOGGER.warn("uuid was not valid in addBalance. Initializing an empty wallet.");
    ...
    public boolean initializeWallet(UUID uuid) {
        if (isValidEntry(uuid)) {
            WBShop.LOGGER.warn("initializeWallet was called, but the wallet for uuid " + uuid.toString() + " is already initialized!");
            return false;
        } else {
            wallets.put(uuid, 0);
            return true;
    ...
    public boolean saveToFile() throws FileNotFoundException {
        ...
        printWriter.write(WBShop.GSON.toJson(this));
        printWriter.close();
        return true;
    }
