...
public class OfferManager {
    public static ArrayList<Offer> getOfferList() {
        return WBShop.SETTINGS.getOfferList();
    }
    ...
    public static void sellScreenClosing(SellScreenHandler handler, PlayerEntity player) {
        ...
        WBShop.LOGGER.info("Player " + player.getName() + " closed sell gui " + handler.syncId);
    }
    ...
    public static void claimPurchases(PlayerEntity player) {
        ...
        getOfferList().forEach(offer -> {
            if (offer.isFilled() && (offer.getOwner().equals(player.getUuid()))) { // TODO: Potentially optimize? I don't know which check is faster in most cases.
                ...
                WBShop.LOGGER.info("Offer " + offer.getId() + " for " + offer.getItemsRequested() + "x " + offer.getItem().getName().getString() + " claimed by player with UUID " + player.getUuid());
                toDelete.add(offer);
            }
        });
    ...
    public static boolean canSell(ItemStack itemStack) {
        if (itemStack.hasNbt()) { // If it has NBT, it cannot be sold
            WBShop.LOGGER.debug("Someone tried to sell " + itemStack + ", but it was rejected because it had NBT data.");
            return false;
        }
