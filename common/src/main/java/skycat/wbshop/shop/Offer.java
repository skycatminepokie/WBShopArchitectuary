...
public class Offer {
    ...
    private static long nextId() {
        return ++WBShop.SETTINGS.lastOfferId; // TODO: Probably should encapsulate the offer id better
    }
    ...
    private void fill(int numOfItems) {
        ...
        if (itemsFilled >= itemsRequested) { // The offer has been filled
            ...
            if (itemsFilled > itemsRequested) { // The offer has been overfilled (try not to let this happen)
                WBShop.LOGGER.warn("itemsFilled (" + itemsFilled + ") is greater than itemsRequested (" + itemsRequested + ") by offer " + id + " (more items were exchanged than requested).");
            }
        }
    }
